...
6. Add guards for optional modules before running them.
   ...
   - Publication bias defaults to a four-part suite, all in the same multilevel model with CR2 robust inference: (a) the multilevel **Egger** test — the **slope** of effect sizes regressed on their standard errors (`yi ~ SE`), a funnel-asymmetry / small-study-effects test; (b) **PET** — the **intercept** of that same `yi ~ SE` model, the bias-adjusted effect at SE = 0; (c) **PEESE** — the **intercept** of `yi ~ variance` (`I(sei^2)`), the bias-adjusted effect when a genuine effect exists; (d) the **conditional PET-PEESE** selection (Stanley & Doucouliagos, 2014). PET and Egger are different coefficients of the same regression (intercept vs slope), so PET is not a duplicate of Egger — do not drop it on that basis, and never relabel the slope and the intercept as the same quantity. See `references/rmarkdown_reporting.md` ("Publication-bias diagnostics") for the canonical code and interpretation rules.
...
7. Run the script and verify the outputs.
   - For expensive or iterative meta-analyses, build a cached pipeline such as `targets` instead of rerunning every stage. Split the workflow into cleaning, model computation, table export, figure rendering, report building, and manifest/provenance targets. Keep expensive model fits separate from report/figure code so layout edits do not refit models, and use a stable output folder for cached pipeline outputs.
...
8. Generate an R Markdown report when the user asks for results, outcomes, Word/PDF output, summary tables, or figures.
   ...
   - Write the manuscript-style Results as journal prose, not a technical dump: sample descriptives -> summary of findings -> overall effect -> moderator analyses -> sensitivity/influence checks -> publication bias, with an explanatory paragraph before each table and the dataset's actual findings (which effects were significant, strongest/weakest levels, direction of continuous moderators), not template filler. Order content as narrative -> matching table -> matching figure.
   - In `results_summary.docx`, write a rich analytic-sample paragraph when the data support it: extracted and model-ready counts, exclusions, study/effect counts per main analysis set, study-level mean-age summary using one averaged age per study, major outcome-category distribution, region/geography distribution, and non-r effect types retained by conversion or extraction coding.
   - After the descriptives, open with a brief Summary of Findings: a compact key-results table (one row per analysis set, family, or main model — k, n, pooled effect with CI, p, prediction interval, Q, I2, tau2/tau components, significant moderators, publication-bias summary) plus a short synthesizing paragraph per set, before the detailed sections. Build it from already-computed result objects, never by refitting. See `references/rmarkdown_reporting.md` ("Summary of Findings").
   - When comparing outcome families that have opposite conceptual directions, add a separate harmonized-direction moderator analysis when requested or needed for interpretation. For example, sign-flip adaptive outcomes so positive effects consistently mean poorer functioning, keep maladaptive outcomes in their original poorer-functioning direction, and test outcome valence as a moderator in the harmonized dataset. Do not replace the original separate-set analyses unless the user asks.
   - When the user needs a manuscript Table 1, create a separate study-characteristics table with one row per study id, preserving multi-study papers as separate rows. Use full citation metadata when available to format author labels, include study design and report type, collapse study-level exposure/outcome dimensions, and put `RoB Score (%)` as the final column. Align sex/gender composition with the analysis moderator: if moderation uses `pct_female`, report `% Female` in Table 1; do not convert it to `% Male` unless the analysis uses a male-coded moderator or the user explicitly requests `% Male`. Score RoB as a percentage: `Yes = 1`, `No`/`Unclear = 0`, and `Not applicable` excluded from the denominator.
   ...
   - Report continuous moderators inside the same moderation table as the categorical ones, using the identical column schema: one omnibus/header row (k, n, pseudo-R2, CR2 slope test) plus `Intercept` and `Slope (per unit)` rows. For report-facing moderator tables, collapse any raw `moderator` + `level` columns into one visible `Moderation` column before Word/HTML/PDF display; never show a separate `moderator` column in manuscript-facing tables. Keep the separate slope-only `continuous_moderators.csv` for full CR2 detail. See `references/rmarkdown_reporting.md` ("Manuscript-style moderator table" and "Continuous moderators in the same moderation table").
   ...
   - Overall/main-effect tables must report prediction intervals and heterogeneity variance components for both the full-data row and any no-outlier/influence sensitivity row. For three-level `rma.mv()` models, extract `tau2_between = fit$sigma2[1]`, `tau2_within = fit$sigma2[2]`, `tau2_total = sum(fit$sigma2)`, and `tau = sqrt(tau2)` for each component; report these beside `Q_statistics` and `I2_total` / `I2_between` / `I2_within`. In the influence/sensitivity section, include a compact table comparing full vs. no-outlier pooled effect, 95% prediction interval, and tau2/tau estimates.
   - When the sensitivity analysis flags influential effects (for example Cook's D > 4/k), include a dedicated table of those flagged effects in the manuscript-facing summary, not just a count: one row per flagged effect with study/effect ids, identifying descriptors, effect size, SE, Cook's D, the cutoff, and the ratio to the cutoff, sorted by Cook's D. Also distinguish studies represented among flagged effects from studies removed entirely: `k_effects` and `n_studies` in main/sensitivity rows are retained model counts, so report flagged effects, studies with flagged effects, studies removed entirely, and effects/studies retained in the sensitivity model. See `references/rmarkdown_reporting.md`.
...
- In cached pipelines, save the actual workflow source files used for the run into `scripts/` or `source/` under the output folder: pipeline definition files, analysis helper files, cleaning scripts, generated `.Rmd` files, and other sourced helpers. Do not archive unrelated scripts as provenance. Clear stale copied source files before writing the current source snapshot, and write `script_manifest.csv`, `report_status.csv`, and `output_manifest.csv`.
...
- For multilevel models, keep tau2/tau extraction tied to the fitted object used for the row being reported. Do not reuse full-data tau estimates for no-outlier/sensitivity rows; recompute prediction intervals and tau2/tau from the corresponding fitted sensitivity model.
...
- Publication-bias coefficients are not interchangeable. The Egger test is the **slope** of `yi ~ SE` (funnel asymmetry); PET is the **intercept** of the same model (bias-adjusted effect at SE = 0); PEESE is the **intercept** of `yi ~ variance`. Extract each by coefficient name (`names(coef(fit))`), not by row position, and never present the Egger slope and the PET intercept as the same number.
- Apply the conditional PET-PEESE rule explicitly: test the PET intercept one-tailed in the direction of the pooled effect at alpha = .05. If significant, a genuine nonzero effect is indicated and the PEESE intercept is the bias-adjusted estimate. If not significant, the conditional conclusion is that the bias-adjusted effect is **not distinguishable from zero**; report the PET intercept and its CI for reference only, never as a point estimate of the true effect (the intercept is an out-of-sample extrapolation to SE = 0 and can fall outside the observed effect range, even opposite in sign to the pooled effect).
- Caveat PET-PEESE under high heterogeneity. When total I^2 is very high (roughly > 90%), PET-PEESE regression adjustments are known to be unreliable and can over-correct (Stanley, 2017); report them qualitatively as evidence about whether a genuine effect remains after small-study adjustment, not as a precise adjusted magnitude.
