...
8. Generate an R Markdown report when the user asks for results, outcomes, Word/PDF output, summary tables, or figures.
   ...
   - After the descriptives, open with a brief Summary of Findings: a compact key-results table (one row per analysis set, family, or main model — k, n, pooled effect with CI, p, prediction interval, I2, significant moderators, publication-bias summary) plus a short synthesizing paragraph per set, before the detailed sections. Build it from already-computed result objects, never by refitting. See `references/rmarkdown_reporting.md` ("Summary of Findings").
