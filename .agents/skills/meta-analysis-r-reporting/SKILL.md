...
8. Generate an R Markdown report when the user asks for results, outcomes, Word/PDF output, summary tables, or figures.
   ...
   - Report continuous moderators inside the same moderation table as the categorical ones, using the identical column schema: one omnibus/header row (k, n, pseudo-R2, CR2 slope test) plus `Intercept` and `Slope (per unit)` rows. Keep the separate slope-only `continuous_moderators.csv` for full CR2 detail. See `references/rmarkdown_reporting.md` ("Continuous moderators in the same moderation table").
