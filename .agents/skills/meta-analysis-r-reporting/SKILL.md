...
8. Generate an R Markdown report when the user asks for results, outcomes, Word/PDF output, summary tables, or figures.
   ...
   - Save every generated table, figure, diagnostic, report, and analysis source file into one clearly named output folder beside the data file. Include at minimum CSV tables, optional XLSX workbook, PNG figures, Word reports, model-status table, the main R script, any helper `.R` or generated `.Rmd` files used for the run, and a plain-text or CSV output manifest that lists those scripts.
   - When producing Word summaries, create exactly two Word documents by default: (1) `results_summary.docx`, with overview, executive summary, key results, and only relevant figures/tables; and (2) `all_tables_figures_summary.docx`, containing all generated result tables plus every generated figure with short labels/captions for checking.
